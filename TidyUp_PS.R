...
tbl_clean <- rename(tbl_clean,
                    id = session_id, # id : session_id (subject number)
                    #gender = genderidentity already recoded gender : genderidentity (gender 1 "Male" 2 "Female" 3 "Trans male/Trans man" 4 "Trans female/Trans woman" 5 "Genderqueer/Gender nonconforming" 6 "A different identity") 
                    race = raceomb_002,# race : raceomb_002 (race: 1 "American Indian" 2 "East Asian" 3 "South Asian" 4 "Hawaiian Pacifica Islander" 5 "black Africian American" 6 "white" 7 "other" 8 "multiracial")
                    bias = D_biep.White_Good_all,# bias :D_biep.White_Good_all (overall IAT score)
                    rt = Mn_RT_all_3467,# rt : Mn_RT_all_3467 (overall reaction time)
                    edu = edu_14,# edu : edu_14 (education: 1 "elementary" 2 "junior high" 3 "some high school" 4 "HS grad" 5 "some college" 6 "associate's" 7 "bachelor's" 8 "some grad" 9 "MA" 10 "JD" 11 "MD" 12 "PHD" 13 "other advanced" 14 "MBA")
                    pol = politicalid_7,# pol : politicalid_7 (political identification: 1 "strongly conservative 7 "strongly liberal)
                    state = STATE,# state : STATE
                    att = att_7,# att : att_7 (race attitude 1 "strongly prefer AA" 7 "strongly prefer white")
                    temp_b = tblacks_0to10,# temp_b : tblacks_0to10 (temperature feelings black 1 "extremely cold" 10 "extremly warm")
                    temp_w = twhites_0to10# temp_w : twhites_0to10 (temperature feelings black 1 "extremely cold" 10 "extremly warm")
)
...
gender_count <- tbl_clean %>% group_by(gender)%>% filter(!is.na(gender)) %>% tally()  
...
tbl_clean1 <- tbl_clean %>% mutate(edu7 = cut(edu, breaks = c(1,2,3,4,5,8,9,13,14),
                                             labels = c("no highschool", "some highschool", "highschool graduate", "some college", "postsecondary degree", "masters (MA & MBA)", "advanced degree", "masters (MA & MBA)")),
                                  gender4 = recode(gender, "[1]" = "Male", "[2]" = "Female", "[5]" = "Genderqueer", .default="Other"))
# filtering and math ---------------------------------------------  

# using filtering, calculate and print the mean bias score for:

# white men
tbl_clean %>% filter(race == 6 & gender == "[1]") %>% summarize(mean(bias))

# white women
tbl_clean %>% filter(race==6 & gender=="[2]") %>% summarize(mean(bias))

# advanced degree holders who are men
tbl_clean %>% filter(edu7=="advanced degree" & gender=="[1]") %>% summarize(mean(bias))

# high school graduates who are men
tbl_clean %>% filter(edu7=="highschool graduate" & gender=="[1]") %>% summarize(mean(bias))
